...
public class TestSinCalculator {

    private static void runSinTestWithPrecision(double arg, double precision) {
        assertEquals(Math.sin(arg), SinCalculator.getInstance().fun(arg), precision);
    }
    ...
    public static Stream<Double> getDataSet() {
        return Stream.of(0d, Math.PI/6, Math.PI/3, Math.PI/2, 2*Math.PI/3, 5*Math.PI/6, Math.PI, 7*Math.PI/6,
